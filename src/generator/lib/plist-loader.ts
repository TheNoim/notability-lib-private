...
import { hexDump } from '../util/hex-dump';
...
export class NotabilitySession {
	...
	public getColorData() {
		...
		for (let i = 0; i < colorEncodedData.length; i += 1) {
			colorNumber.push(colorEncodedData.readUIntLE(i, 1));
		}
		return colorNumber;
	}
	...
	private convertShapeColor(colorObjc: { rgba: [number,number,number,number] }) {
		return {
			r: colorObjc?.rgba?.[0] * 255 ?? 0,
			g: colorObjc?.rgba?.[1] * 255 ?? 0,
			b: colorObjc?.rgba?.[2] * 255 ?? 0,
	...
	public getCoordinates() {
		...
		const curveWidthArray: number[] = [];
		const cgPaths: CGPath[] = [];
		const colorRawData = this.getColorData();
		const colorChunks = chunk(colorRawData, 4) as unknown as Array<number[]>;
		const bezierObject = this.safeReferenceAccess(data.bezierPathsDataDictionary);
		const fractionalWidthArrayArray: number[][] = [];

		const width = this.safeReferenceAccess(data.curveswidth);

		for (let i = 0; i < width.length; i += 4) {
			curveWidthArray.push(width.readFloatLE(i));
		}

		const curvesfractionalwidths = this.safeReferenceAccess(data.curvesfractionalwidths);

		const fractionalWidthMultipliers: number[] = [];

		for (let i = 0; i < curvesfractionalwidths.length; i += 4) {
			fractionalWidthMultipliers.push(curvesfractionalwidths.readFloatLE(i));
		}

		let fracwidthoffset = 0;
		for (const chunk of chunkSizes) {
			const fracChunkSize = Math.floor(chunk / 3) + 1;
			fractionalWidthArrayArray.push(fractionalWidthMultipliers.slice(fracwidthoffset, fracwidthoffset + fracChunkSize));
			fracwidthoffset += fracChunkSize;
		}

		const bezierObjectArray = bezierObject?.["NS.objects"] ?? [];
		const bezierKeyArray = bezierObject?.["NS.keys"] ?? [];

		for (let i = 0; i < bezierObjectArray.length; i++) {
			const ref = bezierObjectArray[i];
			const refKey = this.safeReferenceAccess(bezierKeyArray[i]) as unknown as number;
			const buf = this.safeReferenceAccess(ref);
			const bPath = createBezierKitPath(buf);
			cgPaths[refKey] = bPath;
		}


		for (let i = 0; i < curvesArray.length; i++) {
			const curve = curvesArray[i];
			const width = curveWidthArray[i];
			const colorDataArray = colorChunks[i];
			const cgPath = cgPaths?.[i];
			const fractionalWidthMultipliers = fractionalWidthArrayArray[i];
