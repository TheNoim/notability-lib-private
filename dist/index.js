...
var require_paper_full = __commonJS((exports, module) => {
  var paper = function(self2, undefined) {
    self2 = self2 || require_self();
    var window = self2.window, document2 = self2.document;
    ...
    var Item = Base.extend(Emitter, {
      ...
      copyAttributes: function(source, excludeMatrix) {
        ...
        var data2 = source._data, name = source._name;
        this._data = data2 ? Base.clone(data2) : null;
        if (name)
          this.setName(name);
      },
    ...
    var Raster = Item.extend({
      ...
      getPixel: function() {
        var point = Point.read(arguments);
        var data2 = this.getContext().getImageData(point.x, point.y, 1, 1).data;
        return new Color("rgb", [data2[0] / 255, data2[1] / 255, data2[2] / 255], data2[3] / 255);
      },
      setPixel: function() {
        var args = arguments, point = Point.read(args), color = Color.read(args), components = color._convert("rgb"), alpha = color._alpha, ctx = this.getContext(true), imageData = ctx.createImageData(1, 1), data2 = imageData.data;
        data2[0] = components[0] * 255;
        data2[1] = components[1] * 255;
        data2[2] = components[2] * 255;
        data2[3] = alpha != null ? alpha * 255 : 255;
      ...
      setImageData: function(data2) {
        var point = Point.read(arguments, 1);
        this.getContext(true).putImageData(data2, point.x, point.y);
      },
      _getBounds: function(matrix, options) {
    ...
    var PathItem = Item.extend({
      ...
      statics: {
        create: function(arg) {
          ...
          if (segments) {
            ...
            compound = (data2.match(/m/gi) || []).length > 1 || /z\s*\S+/i.test(data2);
          }
      ...
      setPathData: function(data2) {
        var parts = data2 && data2.match(/[mlhvcsqtaz][^mlhvcsqtaz]*/ig), coords, relative = false, previous, control, current = new Point(), start = new Point();
        ...
        if (match) {
          ...
          if (!color) {
            if (window) {
              ...
              var data2 = colorCtx.getImageData(0, 0, 1, 1).data;
    ...
    var View = Base.extend(Emitter, {
      ...
      return {
        ...
        _handleMouseEvent: function(type, event, point) {
          var itemEvents = this._itemEvents, hitItems = itemEvents.native[type], nativeMove = type === "mousemove", tool2 = this._scope.tool, view = this;
          function responds(type2) {
            return itemEvents.virtual[type2] || view.responds(type2) || tool2 && tool2.responds(type2);
          }
          ...
          lastPoint = point;
          if (handle && tool2) {
            called = tool2._handleMouseEvent(type, event, point, mouse) || called;
          }
        ...
        _handleKeyEvent: function(type, event, key, character) {
          var scope = this._scope, tool2 = scope.tool, keyEvent;
          function emit(obj) {
    ...
    var CanvasView = View.extend({
      ...
      initialize: function CanvasView2(project, canvas2) {
        if (!(canvas2 instanceof window.HTMLCanvasElement)) {
    ...
    var ToolEvent = Event.extend({
      _class: "ToolEvent",
      _item: null,
      initialize: function ToolEvent2(tool2, type, event) {
        this.tool = tool2;
        this.type = type;
        this.event = event;
      },
    ...
    var Tool = PaperScopeItem.extend({
      ...
      _handleMouseEvent: function(type, event, point, mouse) {
        ...
        if (mouse.drag && !this.responds(type))
          type = "mousemove";
        var move = mouse.move || mouse.drag, responds = this.responds(type), minDistance = this.minDistance, maxDistance = this.maxDistance, called = false, tool2 = this;
        function update(minDistance2, maxDistance2) {
          var pt = point, toolPoint = move ? tool2._point : tool2._downPoint || pt;
          if (move) {
            if (tool2._moveCount >= 0 && pt.equals(toolPoint)) {
              return false;
            }
        ...
        function emit() {
          if (responds) {
            called = tool2.emit(type, new ToolEvent(tool2, type, event)) || called;
          }
        }
        if (mouse.down) {
          update();
    ...
    new function() {
      ...
      function exportCompoundPath(item, options) {
        ...
        var data2 = item.getPathData(null, options.precision);
        if (data2)
          attrs.d = data2;
      ...
      function exportSVG(item, options, isRoot) {
        ...
        if (node) {
          ...
          var data2 = JSON.stringify(item._data);
          if (data2 && data2 !== "{}" && data2 !== "null")
            node.setAttribute("data-paper-data", data2);
        }
      ...
      function importNode(node, options, isRoot) {
        ...
        if (item) {
          if (isElement && !(item instanceof Group))
            item = applyAttributes(item, node, isRoot);
          var onImport = options.onImport, data2 = isElement && node.getAttribute("data-paper-data");
          if (onImport)
    ...
    Base.exports.PaperScript = function() {
      ...
      function execute(code, scope, options) {
        paper2 = scope;
        var view = scope.getView(), tool2 = /\btool\.\w+|\s+on(?:Key|Mouse)(?:Up|Down|Move|Drag)\b/.test(code) && !/\bnew\s+Tool\b/.test(code) ? new Tool() : null, toolHandlers = tool2 ? tool2._events : [], handlers = ["onFrame", "onResize"].concat(toolHandlers), params = [], args = [], func, compiled = typeof code === "object" ? code : compile(code, options);
        code = compiled.code;
        ...
        expose({__$__, $__, paper: scope, tool: tool2}, true);
        expose(scope);
...
// src/client/index.ts
var import_paper = __toModule(require_paper_full());

// src/generator/lib/INotabilitySession.ts
var NotabilityDrawInstructionType;
(function(NotabilityDrawInstructionType2) {
  NotabilityDrawInstructionType2[NotabilityDrawInstructionType2["CURVE"] = 0] = "CURVE";
  NotabilityDrawInstructionType2[NotabilityDrawInstructionType2["SHAPE"] = 1] = "SHAPE";
  NotabilityDrawInstructionType2[NotabilityDrawInstructionType2["MEDIA"] = 2] = "MEDIA";
})(NotabilityDrawInstructionType || (NotabilityDrawInstructionType = {}));

// src/generator/lib/IShapes.ts
var NotabilityShapeType;
(function(NotabilityShapeType2) {
  NotabilityShapeType2["LINE"] = "line";
  NotabilityShapeType2["RECTANGLE"] = "rectangle";
  NotabilityShapeType2["CIRCLE"] = "circle";
  NotabilityShapeType2["TRIANGLE"] = "triangle";
  NotabilityShapeType2["ELLIPSE"] = "ellipse";
  NotabilityShapeType2["PARTIALSHAPE"] = "partialshape";
})(NotabilityShapeType || (NotabilityShapeType = {}));
var CGPathInstructionType;
(function(CGPathInstructionType2) {
  CGPathInstructionType2[CGPathInstructionType2["MOVE_TO"] = 0] = "MOVE_TO";
  CGPathInstructionType2[CGPathInstructionType2["ADD_LINE"] = 1] = "ADD_LINE";
  CGPathInstructionType2[CGPathInstructionType2["ADD_QUAD_CURVE"] = 2] = "ADD_QUAD_CURVE";
  CGPathInstructionType2[CGPathInstructionType2["ADD_CURVE"] = 3] = "ADD_CURVE";
})(CGPathInstructionType || (CGPathInstructionType = {}));

// src/client/index.ts
var canvas = document.querySelector("#notability-canvas");
import_paper.default.setup(canvas);
canvas.addEventListener("wheel", (event) => {
  let newZoom = import_paper.default.view.zoom;
  let oldZoom = import_paper.default.view.zoom;
  if (event.deltaY > 0) {
    newZoom = import_paper.default.view.zoom * 1.05;
  } else {
    newZoom = import_paper.default.view.zoom * 0.95;
  }
  let beta = oldZoom / newZoom;
  let mousePosition = new import_paper.default.Point(event.offsetX, event.offsetY);
  let viewPosition = import_paper.default.view.viewToProject(mousePosition);
  let mpos = viewPosition;
  let ctr = import_paper.default.view.center;
  let pc = mpos.subtract(ctr);
  let offset = mpos.subtract(pc.multiply(beta)).subtract(ctr);
  import_paper.default.view.zoom = newZoom;
  import_paper.default.view.center = import_paper.default.view.center.add(offset);
  event.preventDefault();
});
var tool = new import_paper.default.Tool();
tool.onMouseDrag = function(event) {
  const pan_offset = event.point.subtract(event.downPoint);
  import_paper.default.view.center = import_paper.default.view.center.subtract(pan_offset);
};
var resp = await fetch("data.json");
var data = await resp.json();
console.log(data);
function createPointFromIPoint(point) {
  return new import_paper.default.Point(point.x, point.y);
}
function drawCGPath(cgPath) {
...
function createPaperColor(color) {
  return new import_paper.default.Color(color.r / 255, color.g / 255, color.b / 255, color.a);
}
var mediaCount = 0;
for (const instruction of data) {
  switch (instruction["@NType"]) {
    case NotabilityDrawInstructionType.CURVE: {
      const coordinateData = instruction;
    ...
    case NotabilityDrawInstructionType.MEDIA: {
      ...
      const raster = new import_paper.default.Raster(imgTag);
      raster.position = new import_paper.default.Point(media.origin.x + media.dimension.width / 2, media.origin.y + media.dimension.height / 2);
      raster.onLoad = () => {
    ...
    case NotabilityDrawInstructionType.SHAPE: {
      ...
      switch (shape.type) {
        case NotabilityShapeType.PARTIALSHAPE: {
          ...
          if (partialShape.style) {
            if (partialShape.style.fillColor) {
              ...
              path.fillColor = import_paper.default.Color.random();
            }
          } else {
            path.fillColor = import_paper.default.Color.random();
          }
        ...
        case NotabilityShapeType.CIRCLE: {
          ...
          const path = new import_paper.default.Path.Circle(center, circle.radius);
        ...
        case NotabilityShapeType.ELLIPSE: {
          const ellipse = shape;
          const origin = new import_paper.default.Point(ellipse.center.x - ellipse.radiusX, ellipse.center.y - ellipse.radiusY);
          const rectangle = new import_paper.default.Rectangle(origin, new import_paper.default.Size(ellipse.radiusX * 2, ellipse.radiusY * 2));
          const path = new import_paper.default.Path.Ellipse(rectangle);
